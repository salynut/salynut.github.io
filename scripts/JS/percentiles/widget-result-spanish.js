...
var widget_result = function($, moment, widgetData, queryStr) {
...
$('#link_to_res').attr('href', 'https://www.cdc.gov/healthyweight/bmi/'+queryStr)
...
/*  function getQueryStrParamByName(name, url) {
  ...
  */
  ...
  //console.log('Troubleshoot: What is age_m ', age_m)
...
//console.log(age_y, age_m, gender, hinches, method, totalWeightPounds)
  ...
  if ( isDataInQueryString() ) { // there are inputs
    ...
    // display entered sex
    $('#sex_m_f').text(gender==='m'?'niño':'niña')
    ...
    */
    ...
    switch (bmiCalculatedObj.z_perc) {
      ...
      case 0:
        $('.bmi_percentile').html('percentil menor a 1 <sup>st</sup> ')
        break

      case 100:
        $('.bmi_percentile').html('percentil mayor a 99 <sup>th</sup> ')
        break
  ...
  function getBMIConclusion(z_percentile) {
    ...
    if (z_percentile < 5) {
      return 'se encuentra con bajo peso'
    } else if ( 5 <= z_percentile && z_percentile < 85 ) {
      return 'se encuentra en un peso saludable'
  ...
  function getBMI_text(z_percentile) {
    ...
    if (z_percentile < 5) {
      return ' y debe ser visto por un proveedor de atención médica para una evaluación adicional para determinar las posibles causas de bajo peso'
    } else if ( 5 <= z_percentile && z_percentile < 85 ) {
      return ''
    } else if ( 85 <= z_percentile && z_percentile < 95 ) {
      return ''
	  ...
	  var tempRetTxt = ' y puede tener problemas de salud relacionados con el peso, '
      var tempGend = 'el'
      if (gender==='f') { tempGend = 'ella' }
      tempRetTxt += tempGend
      tempRetTxt += ' debe ser visto por un profesional de la salud para una evaluación adicional'

//console.log('This is the text:', tempRetTxt)
      return tempRetTxt
